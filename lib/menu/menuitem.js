...
class CETMenuItem extends lifecycle_1.Disposable {
    ...
    render(container) {
        ...
        this._register((0, dom_1.addDisposableListener)(this.container, dom_1.EventType.MOUSE_DOWN, e => {
            if (this.item.enabled && e.button === 0 && this.container) {
                (0, dom_1.addClass)(this.container, 'active');
            }
        }));
        this._register((0, dom_1.addDisposableListener)(this.container, dom_1.EventType.CLICK, e => {
            if (this.item.enabled) {
                this.onClick(e);
            }
        }));
        this._register((0, dom_1.addDisposableListener)(this.container, dom_1.EventType.DBLCLICK, e => {
            dom_1.EventHelper.stop(e, true);
        }));
        [dom_1.EventType.MOUSE_UP, dom_1.EventType.MOUSE_OUT].forEach(event => {
            this._register((0, dom_1.addDisposableListener)(this.container, event, e => {
                dom_1.EventHelper.stop(e);
                (0, dom_1.removeClass)(this.container, 'active');
            }));
        });
        this.itemElement = (0, dom_1.append)(this.container, (0, dom_1.$)('a.action-menu-item'));
        this.itemElement.setAttribute('role', 'menuitem');
        if (this.mnemonic) {
            this.itemElement.setAttribute('aria-keyshortcuts', `${this.mnemonic}`);
        }
        this.checkElement = (0, dom_1.append)(this.itemElement, (0, dom_1.$)('span.menu-item-check'));
        this.checkElement.setAttribute('role', 'none');
        this.iconElement = (0, dom_1.append)(this.itemElement, (0, dom_1.$)('span.menu-item-icon'));
        this.iconElement.setAttribute('role', 'none');
        this.labelElement = (0, dom_1.append)(this.itemElement, (0, dom_1.$)('span.action-label'));
        this.setAccelerator();
        this.updateLabel();
        this.updateIcon();
        this.updateTooltip();
    ...
    onClick(event) {
        ...
        if (this.item.role) {
            electron_1.ipcRenderer.send('execute-menu-command', {
                role: this.item.role
            });
        }
    ...
    focus() {
        if (this.container) {
            ...
            (0, dom_1.addClass)(this.container, 'focused');
        }
        this.applyStyle();
    }
    blur() {
        if (this.container) {
            this.container.blur();
            (0, dom_1.removeClass)(this.container, 'focused');
        }
        this.applyStyle();
    }
    setAccelerator() {
        ...
        if (accelerator !== null) {
            (0, dom_1.append)(this.itemElement, (0, dom_1.$)('span.keybinding')).textContent = parseAccelerator(accelerator);
        }
    }
    updateLabel() {
        if (this.item.label) {
            let label = this.item.label;
            if (label) {
                const cleanLabel = (0, menu_1.cleanMnemonic)(label);
    ...
    updateIcon() {
        ...
        if (icon) {
            const iconE = (0, dom_1.append)(this.iconElement, (0, dom_1.$)('img'));
    ...
    updateEnabled() {
        if (this.item.enabled && this.item.type !== 'separator') {
            (0, dom_1.removeClass)(this.container, 'disabled');
            this.container.tabIndex = 0;
        }
        else {
            (0, dom_1.addClass)(this.container, 'disabled');
        }
    }
    updateVisibility() {
    ...
    dispose() {
        if (this.itemElement) {
            (0, dom_1.removeNode)(this.itemElement);
